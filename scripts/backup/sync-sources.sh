...
# Run from the codex repo root.

set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BACKUPS_ROOT="${ROOT}/backups/projects"
...
if [[ -d "${HOME_DIR}/home-assistant-dashboard" ]]; then
  mkdir -p "${BACKUPS_ROOT}/home-assistant-dashboard"
  rsync -a --delete \
    --exclude='.git' --exclude='.DS_Store' \
    "${HOME_DIR}/home-assistant-dashboard/" "${BACKUPS_ROOT}/home-assistant-dashboard/"
  echo "  ✓ home-assistant-dashboard"
fi

# n8n Prompt Manager (exclude large binaries)
if [[ -d "${HOME_DIR}/n8n-prompt-manager" ]]; then
  mkdir -p "${BACKUPS_ROOT}/n8n-prompt-manager"
  rsync -a --delete \
    --exclude='.git' --exclude='.DS_Store' \
    --exclude='*.zip' --exclude='*.app' --exclude='*.dmg' \
    --exclude='app.log' --exclude='__pycache__' \
    --exclude='windows-package' \
    "${HOME_DIR}/n8n-prompt-manager/" "${BACKUPS_ROOT}/n8n-prompt-manager/"
...
for proj in cardsaas pharmatech pharmatech-mobile; do
  if [[ -d "${CURSOR_FULLSTACK}/${proj}" ]]; then
    mkdir -p "${BACKUPS_ROOT}/${proj}"
    rsync -a --delete \
      --exclude='.git' --exclude='.DS_Store' \
      --exclude='node_modules' --exclude='.next' --exclude='out' \
      --exclude='.playwright-cli' \
      "${CURSOR_FULLSTACK}/${proj}/" "${BACKUPS_ROOT}/${proj}/"
    echo "  ✓ ${proj}"
  fi
done
