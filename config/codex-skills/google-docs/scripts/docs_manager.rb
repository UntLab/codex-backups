...
class DocsManager
  ...
  def authorize
    ...
    if credentials_json['type'] == 'service_account'
      # Service account flow: uses key directly and skips interactive OAuth.
  ...
  def complete_auth(code)
    ...
    if credentials_json['type'] == 'service_account'
      output_json({
        status: 'success',
        message: 'Service account credentials detected. OAuth code flow is not required.',
    ...
    if code.nil? || code.strip.empty?
      output_json({
        ...
        message: 'Authorization code required for OAuth credentials',
  ...
  def parse_markdown(markdown)
    ...
    while i < lines.length
      ...
      if line.start_with?('#### ')
        heading_text = line[5..] + "\n"
        formats << { type: :heading4, start: current_index, end: current_index + heading_text.length - 1 }
        text += heading_text
    ...
    when :heading4
      {
        update_paragraph_style: {
          range: { start_index: fmt[:start], end_index: fmt[:end] },
          paragraph_style: { named_style_type: 'HEADING_4' },
          fields: 'namedStyleType'
        }
      }
    when :code
